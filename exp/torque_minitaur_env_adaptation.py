...
class Cost(object):
    ...
    def cost_fn(self, samples):
        ...
        if self.__hard_smoothing:
            if self.__action_space == "Torque":
                for t in range(3, int(self.__sol_dim / ad) + 3):
                    amax = a[:, (t - 1) * ad:t * ad] + self.__max_torque_jerk
                    amin = a[:, (t - 1) * ad:t * ad] - self.__max_torque_jerk
                    amax, amin = torch.clamp(amax, self.__lb, self.__ub), torch.clamp(amin, self.__lb, self.__ub)
                    m = torch.distributions.uniform.Uniform(amin - 0.001, amax + 0.001,
                                                            (self.__max_iters * self.__popsize, ad))
                    a[:, t * ad:(t + 1) * ad] = m.sample()
            elif self.__action_space == "velocity":
                for t in range(3, int(self.__sol_dim / ad) + 3):
        ...
        for i in range(n_batch):
            ...
            for h in range(index_offset, self.__horizon + index_offset):
                ...
                if self.__action_space == "velocity2":
                    actions[:, :4] = actions[:, :4] * 0.6
                    actions[:, 4:] = (actions[:, 4:] * 0.4 + 0.5) * 0.75 + 0.10
                    actions = self.transform_action_to_motor_command(actions)
                    actions = (actions - start_states[:, :8])
                elif self.__action_space == "Motor2":
                    actions[:, :4] = actions[:, :4] * 0.6
                    actions[:, 4:] = (actions[:, 4:] * 0.4 + 0.5) * 0.75 + 0.10
                    actions = self.transform_action_to_motor_command(actions) - 1.5
                elif self.__action_space == "Torque2":
                    actions[:, :4] = actions[:, :4] * 0.6
                    actions[:, 4:] = (actions[:, 4:] * 0.4 + 0.5) * 0.75 + 0.10
                    actions = self.transform_action_to_motor_command(actions)
                    actions = 8 * (actions - start_states[:, :8])
...
def execute_random(env, steps, init_state, K, index_iter, res_dir, samples, config):
    ...
    for i in range(3, steps + 3):
        if config['hard_smoothing']:
            if config['action_space'] in ["Motor", "Motor2", "velocity2", "S&E", 'Torque2']:
                ...
                amax = past[i - 1] + max_torque_jerk
                amin = past[i - 1] - max_torque_jerk
            elif config['action_space'] == "velocity":
                amax = np.min((past[i - 1] + max_vel, 2 * past[i - 1] - past[i - 2] + max_acc), axis=0)
                amin = np.max((past[i - 1] - max_vel, 2 * past[i - 1] - past[i - 2] - max_acc), axis=0)
            amax, amin = np.clip(amax, lb, ub), np.clip(amin, lb, ub)
            x = np.random.uniform(amin, amax)
            a = np.copy(x)
        else:
            a = np.random.uniform(lb, ub, 8)
        ...
        desired_torques = desired_torques + list(info['desired_torques'])
        observed_torques = observed_torques + list(info['observed_torques'])
...
config = {
    ...
    "action_dim": 8,
    "action_space": ['S&E', 'Motor', 'Motor2', 'velocity', 'velocity2', 'Torque', 'Torque2'][0],
    ...
    "result_dir": "results",
    "env_name": "torque_minitaur_03",
    "exp_suffix": "experiment",
    "exp_dir": None,
    "exp_details": "Controlled Minitaur evaluate from scratch",
    "dump_trajects": 1,
    "data_dir": "",
    "logdir": None,
    ...
    "cuda": True,
    "ensemble_epoch": 5,
    "ensemble_dim_in": 25 + 8-1,
    "ensemble_dim_out": 25-1,
    ...
    "max_torque_jerk": 25,
    "popsize": 100,
    "sol_dim": 8 * 25,  # NOTE: Depends on Horizon
    "num_elites": 50,
    "cost_fn": None,
    "alpha": 0.1,
    "discount": 1.
...
def check_config(config):
    ...
    config["max_torque_jerk"] = config["max_torque_jerk"] * config['ctrl_time_step']
    config['partial_torque_control'] = config['partial_torque_control'] or (
                config['action_space'] in ["Torque", "Torque2"])
    # if config['action_space'] == "Torque":
    #     config['vkp'] = 20
...
def env_args_from_config(config):
    return {'control_time_step': config['ctrl_time_step'],
            'action_repeat': int(250 * config['ctrl_time_step']),
            'accurate_motor_model_enabled': (config['action_space'] != 'Torque') * 0,
            'pd_control_enabled': 1,
            'partial_torque_control': config['partial_torque_control'] or (
                        config['action_space'] in ["Torque", "Torque2"]),

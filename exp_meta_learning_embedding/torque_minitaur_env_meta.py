...
class Cost(object):
    def __init__(self, model, init_state, horizon, action_dim, goal, pred_high, pred_low, config, last_action, task_likelihoods):
        ...
        self.__desired_speed = config['desired_speed']
    ...
    def cost_fn(self, samples):
        ...
        if self.__hard_smoothing:
            for t in range(3, int(self.__sol_dim / ad) + 3):
                amax = torch.min(a[:, (t-1)*ad:t*ad] + self.__max_vel,
                                 2*a[:, (t-1)*ad:t*ad] - a[:, (t-2)*ad:(t-1)*ad] + self.__max_acc, out=None)
                amax = torch.min(amax,
                                 3*a[:, (t-1)*ad:t*ad] - 3*a[:, (t-2)*ad:(t-1)*ad] + a[:, (t-3)*ad:(t-2)*ad] + self.__max_jerk,
                                 out=None)
                ...
                amin = torch.max(amin,
                                 3*a[:, (t-1)*ad:t*ad] - 3*a[:, (t-2)*ad:(t-1)*ad] + a[:, (t-3)*ad:(t-2)*ad] - self.__max_jerk,
                                 out=None)
                amax, amin = torch.clamp(amax, self.__lb+0.001, self.__ub), torch.clamp(amin, self.__lb, self.__ub-0.001)
                m = torch.distributions.uniform.Uniform(amin, amax, (self.__max_iters * self.__popsize, ad))
        ...
        for i in range(n_batch):
            ...
            for h in range(index_offset, self.__horizon+index_offset):
                ...
                if self.__action_space == "deltaMotor":
                    actions = (1.5 + actions * np.pi / 4 - start_states[:, :8]) * 4/np.pi
                ...
                # x_vel_cost = torch.abs(start_states[:, -3] - self.__desired_speed) * self.__xreward
                z_cost = (start_states[:, -1] - 0.17)**2 * self.__zreward
                roll_cost = (start_states[:, 16])**2 * self.__rollreward
                pitch_cost = (start_states[:, 17]) ** 2 * self.__pitchreward
                yaw_cost = (start_states[:, 18]) ** 2 * self.__yawreward

                if self.__soft_smoothing:
                    action_vel_cost = (action_batch[:, h * self.__action_dim: (h+1) * self.__action_dim] - action_batch[:, (h-1) * self.__action_dim: h * self.__action_dim])**2

                    all_costs[start_index: end_index] += torch.sum(action_vel_cost, axis=1) * self.__action_vel_w * self.__discount ** h
                    action_acc_cost = (action_batch[:, h * self.__action_dim: (h + 1) * self.__action_dim]
                                       - 2*action_batch[:, (h - 1) * self.__action_dim: h * self.__action_dim]
                                       + action_batch[:, (h-2) * self.__action_dim: (h - 1) * self.__action_dim]) ** 2
                    all_costs[start_index: end_index] += torch.sum(action_acc_cost, axis=1) * self.__action_acc_w * self.__discount ** h
...
def execute(env, init_state, steps, init_mean, init_var, model, config, last_action_seq, pred_high,
            pred_low, K, index_iter, final_iter, samples, task_likelihoods):
    if config['record_video'] and index_iter % config['video_recording_frequency'] == 0 or index_iter == final_iter-1:
    ...
    obs, acs, reward, rewards = [current_state], [], [], []

    for t in range(steps):
        if t == 0:
            virtual_acs = list([0]*8) + list([0]*8) + list([0]*8)
        elif t == 1:
            virtual_acs = list([0]*8) + list([0]*8) + list(acs[-1])
        elif t == 2:
            virtual_acs = list([0]*8) + list(acs[-2]) + list(acs[-1])
        else:
            virtual_acs = list(acs[-3]) + list(acs[-2]) + list(acs[-1])
        ...
        cost_object = Cost(model=model, init_state=current_state, horizon=config["horizon"],
                           ...
                           pred_low=pred_low, config=config, last_action=virtual_acs, task_likelihoods=task_likelihoods)
        config["cost_fn"] = cost_object.cost_fn
        optimizer = RS_opt(config)

        sol = optimizer.obtain_solution(acs=virtual_acs)
        a = sol[0:env.action_space.shape[0]]
        if config['action_space'] == "deltaMotor":
            a = (np.ones(8) * 1.5 + a * np.pi / 4 - current_state[:8]) * 4/np.pi
...
def main(gym_args, config, test_mismatch, index, gym_kwargs={}):
    ...
    for index_iter in tbar:
        ...
        data += np.concatenate((data, trajectory), axis=0)
...
config = {
    ...
    "action_dim": 8,
    "action_space": ['Motor', 'deltaMotor', 'S&E'][0],  # choice of action space between Motor joint, swing and extension of each leg and delta motor joint
    ...
    # logging
    "result_dir": "results",
    "data_dir": "data/torque_minitaur/friction_MPC/run_0",
    "model_name": "minitaur_minitaur_meta_embedding_model",
    "env_name": "meta_torque_minitaur_02",
    ...
    "max_action_velocity": 0.6,  # 0.3 from working controller
    "max_action_acceleration": 0.6,  # 0.5 from working controller
    "max_action_jerk": 0.6,  # ? from working controller
    "num_elites": 30,
    "cost_fn": None, 
    "alpha": 0.1,
    "discount": 1.0
...
test_mismatches = np.array([[0, 1, 0]])

args = ["MinitaurTorqueEnv_fastAdapt-v0"]
kwargs = {'control_time_step': config['ctrl_time_step'],
          'action_repeat': int(250*config['ctrl_time_step']),

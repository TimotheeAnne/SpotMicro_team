...
class FFNN_Meta(nn.Module):
    """
    simple FFNN_Meta with dropout regularization
    """
    def __init__(self, dim_in, hidden, dim_out, CUDA=False, SEED=None, output_limit=None, dropout=0, hidden_activation="tanh"):

        super(FFNN_Meta, self).__init__()
        self.dim_in = dim_in
    ...
    def predict(self, x):
        ...
        # De-normalize 
        return (self.forward(x_tensor) * self.data_std_output + self.data_mean_output).detach().cpu().numpy()
    ...
    def predict_tensor(self, x):
        ...
        # De-normalize 
        return (self.forward(x_normalized) * self.data_std_output + self.data_mean_output).detach()
...
def train(model, data_in, data_out, inner_iter=100, inner_lr=1e-3, minibatch=32, optimizer=None):
...
def gen_test_task():
    phase = np.random.uniform(low=-0.4, high=0.4*np.pi)
...
if __name__ == '__main__':    
    ...
    for i in range(num_tasks):
        x = np.linspace(-6, 6, np.random.randint(30, 40)).reshape(-1,1)
        f = gen_test_task()
        y = f(x)
        tasks_in.append(x)
        tasks_out.append(y)
    ...
    '''-----------Meta learning------------'''
    model = FFNN_Meta(dim_in=1, hidden=[20, 20, 20], dim_out=1, CUDA=True, SEED=None, output_limit=None, dropout=0.0)
    train_meta(model, tasks_in, tasks_out, meta_iter=meta_iter, inner_iter=inner_iter, inner_step=n_step, meta_step=m_step, minibatch=32, inner_sample_size=64)    
    ...
    predict_before = model.predict_tensor(torch.Tensor(test_in).cuda()).data.cpu().numpy()
    plt.plot(test_in, predict_before, '--b', alpha=0.5, label="Before training")

    '''----------Train model with optimized init params-----------'''
    train(model, train_in, train_out, inner_iter=1000, inner_lr=1e-3, minibatch=32)
    
    # Model after training with data
    predict_after = model.predict_tensor(torch.Tensor(test_in).cuda()).data.cpu().numpy()
    ...
    '''--------------No meta leaning-----------------------'''
    model2 = FFNN_Meta(dim_in=1, hidden=[20,20, 20], dim_out=1, CUDA=False, SEED=None, output_limit=None, dropout=0.0)
    train(model2, train_in, train_out, inner_iter=1000, inner_lr=1e-3, minibatch=32)
    # tasks_tensor = torch.LongTensor([[0] for _ in range(len(test_in))]).cuda() if model2.cuda_enabled else torch.LongTensor([[0] for _ in range(len(test_in))])
    predict_after = model2.predict_tensor(torch.Tensor(test_in)).data.numpy()

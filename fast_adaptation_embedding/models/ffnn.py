...
class FFNN_Model():
    def __init__(self, dim_in, hidden, dim_out, CUDA=False, SEED=None, output_limit=None, dropout=0.0,
        ...
        self.CUDA = CUDA
        self.output_limit = output_limit
        self.model = FFNN(dim_in=dim_in, hidden=hidden, dim_out=dim_out, CUDA=CUDA, SEED=SEED,
    ...
    def train(self, epochs, training_inputs, training_targets, batch_size=None, logInterval=100):
        ...
        self.data_std_input = torch.FloatTensor(
            np.std(training_inputs, axis=0)).cuda() + 1e-10 if self.CUDA else torch.FloatTensor(
        ...
        self.data_std_output = torch.FloatTensor(
            np.std(training_targets, axis=0)).cuda() + 1e-10 if self.CUDA else torch.FloatTensor(
    ...
    def predict(self, d_in):  # TODO: this is not efficient due to the conversions from numpy to tensor
       ...
       d_in: 2d numpy arrays
        """
        x = (torch.FloatTensor(d_in).cuda() - self.data_mean_input) / self.data_std_input if self.CUDA else (
                                                                                                                        torch.FloatTensor(
                                                                                                                            d_in) - self.data_mean_input) / self.data_std_input
    ...
    def save(self, file_path):
        ...
        torch.save({"others": others, "state_dict": state_dict}, file_path)

    def load(self, file_path, device):
        model_data = torch.load(file_path, map_location=device)
        self.data_mean_input = model_data["others"]["data_mean_input"]
        self.data_std_input = model_data["others"]["data_std_input"]
        self.data_mean_output = model_data["others"]["data_mean_output"]
        self.data_std_output = model_data["others"]["data_std_output"]
        self.model.load_state_dict(model_data["state_dict"])


class FFNN_Ensemble_Model():
    def __init__(self, dim_in, hidden, dim_out, n_ensembles, CUDA=False, SEED=None, output_limit=None, dropout=0.0,
                 hidden_activation="tanh"):
        ...
        for i in range(self.n_ensembles):
            self.models.append(FFNN_Model(dim_in=dim_in, hidden=hidden, dim_out=dim_out, CUDA=CUDA, SEED=SEED,
                                          output_limit=output_limit, dropout=dropout,
                                          hidden_activation=hidden_activation))

    def save(self, file_path):
        for i in range(self.n_ensembles):
            self.models[i].save(file_path+"FFNN_Model_"+str(i)+'.pt')
        ...
        torch.save({"kwargs": kwargs}, file_path+"FFNN_Ensemble_Model.pt")
...
def load_model(file_path, device=torch.device('cpu')):
    model_data = torch.load(file_path+"FFNN_Ensemble_Model.pt", map_location=device)
    model_data["kwargs"]["CUDA"] = True if device == torch.device('cuda') else False
    model = FFNN_Ensemble_Model(**model_data["kwargs"])
    for i in range(model.n_ensembles):
        model.models[i].load(file_path + "FFNN_Model_" + str(i) + '.pt', device)

...
class Minitaur(object):
  ...
  def SetInitOrientation(self, orientation):
    self._init_orientation = orientation

  def Step(self, action):
    obs_torques, desired_torques = [], []
    for i in range(self._action_repeat):
      if self._partial_torque_control:
        a = self._past_action + (i+1)/self._action_repeat * (action-self._past_action)
      else:
        a = np.copy(action)
      self.ApplyAction(a)
      obs_torques.append(self._observed_motor_torques)
      desired_torques.append(np.copy(a))
      self._pybullet_client.stepSimulation()
      self.ReceiveObservation()
      self._step_counter += 1
    self._past_action = np.copy(action)
    return obs_torques, desired_torques
  ...
  def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    if not self._torque_control_enabled and reset_time > 0.0:
      ...
      self._initiated = False
      for _ in range(100):
        self.ApplyAction([np.pi/2] * self.num_motors)
        self._pybullet_client.stepSimulation()
        self.ReceiveObservation()
  ...
  def GetBaseVelocity(self):
    """Get the velocity of minitaur's base.
  ...
  def ApplyAction(self, motor_commands, motor_kps=None, motor_kds=None):
    ...
    if motor_kds is None:
      ...
      if self._accurate_motor_model_enabled:
        ...
        if self._partial_torque_control and self._initiated:
          """ virtual bound of the joint in the motor spaces """
          # q0_up = (np.ones(8) * 1.5 + 0.6 * np.pi / 4)
          # q0_low = (np.ones(8) * 1.5 - 0.6 * np.pi / 4)
          # virtual_upper_action = (q > q0_up) * (vkp * (q0_up - q) - vkd * qdot)
          # virtual_lower_action = (q < q0_low) * (vkp * (q0_low - q) - vkd * qdot)

          """ virtual of bound of the joint in the swing&extension space """
          s = np.repeat(q[::2]-q[1::2], 2)
          e = np.repeat(q[::2]+q[1::2], 2)
          alpha = 0.9
          s_up, s_low = (np.full(8, 0.94))*alpha, (np.full(8, -0.94))*alpha
          e_up, e_low = (np.full(8, 3.18+alpha*0.48)), (np.full(8, 3.18-alpha*0.48))
          s_trans = np.array([1, -1, 1, -1, 1, -1, 1, -1])
          virtual_upper_action = (e > e_up) * (self._vkp * (e_up - e)) + (s > s_up) * (self._vkp * (s_up - s)) * s_trans
          virtual_lower_action = (e < e_low) * (self._vkp * (e_low - e)) + (s < s_low) * (self._vkp * (s_low - s)) * s_trans

          damping_action = - 0.3 * qdot
          torque_commands = motor_commands + damping_action + virtual_upper_action + virtual_lower_action
        else:
          torque_commands = -1 * motor_kps * (q - motor_commands) - motor_kds * qdot
        # The torque is already in the observation space because we use
        # GetMotorAngles and GetMotorVelocities.
        torque_commands = np.clip(torque_commands, np.full(8, -self._max_torque), np.full(8, self._max_torque))
